...
{
    unsigned long int id = *(int *)vargp;
	
	// Create a static variable to observe its changes
    // static indica que s é uma variavel global só acessada pelas threads
    static int s = 0;
    ...
    printf("Thread ID(%p): %lu, Static(%p): %d, Global(%p): %d\n", 
                      &id, id,        &s, ++s,        &g, ++g);
