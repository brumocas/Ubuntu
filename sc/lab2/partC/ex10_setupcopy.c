...
pthread_mutex_t buff1_mutex, buff2_mutex, aux_mutex;
pthread_cond_t buff1_prod_condv, buff2_prod_condv, aux_condv;
...
void* producer(void *arg)
{
    ...
    while (1) {
        ...
        if ( pthread_mutex_lock(&aux_mutex) == 0 )
        { 
            while (!circ_buff_isempty(&buffer2))
            {
                pthread_cond_wait(&aux_condv, &aux_mutex);
            }
            ...
            if ( pthread_mutex_lock(&buff1_mutex) == 0 ) {
                ...
                else
                {
                    ...
                    //signal relay to pop from buffer1
...
void* relay(void *arg)
{
    ...
    while (1) {
        if ( pthread_mutex_lock(&buff1_mutex) == 0 )
        {
            ...
            //Wait for signal to pop from buffer1
            ...
            if ( pthread_mutex_lock(&buff2_mutex) == 0 ) {
                ...
                if (circ_buff_pop(&buffer1, &pop_value)==0) {
                    ...
                    if (circ_buff_push(&buffer2, pop_value ) == 0)
                    {
                        ...
                        fprintf(stderr, "Buffer 2 is Full - Droped value: %d\n", pop_value);//Signal to consumer to pop from buffer 2
    ...
    while (1) {
        ...
        if (suspend_f)
            ...
            if ( pthread_mutex_lock(&buff2_mutex) == 0 ) {
                ...
                //Wait for signal relay to pop from buffer2
...
int main(void)
{
    ...
    if (pthread_mutex_init(&aux_mutex, NULL) !=0 ){
        fprintf(stderr, "Error in pthread_mutex_init ");
        return 1;
    }

    if (pthread_cond_init(&aux_condv, NULL) !=0 ){
        fprintf(stderr, "Error in pthread_cond_init ");
        return 1;
    }
